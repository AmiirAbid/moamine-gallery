<script>
	import { Phone } from '@lucide/svelte';
</script>

<div class="flex h-screen items-center justify-center text-center text-foreground flex-col gap-6 tracking-[-3%] mx-6">
	<h2 class="text-5xl font-semibold">Coming Soon</h2>
	<div class="flex flex-col items-center gap-4">
		<h3 class="text-2xl font-light">Currently available for custom projects</h3>
		<a href="tel:+21697494493" class="bg-foreground dark:hover:bg-white/50 hover:bg-black/50 text-background px-4 py-2 rounded-full text-xl font-normal flex gap-2 items-center">
			<Phone class="w-5 h-5 cursor-pointer"/>
			Reach Out
		</a>
	</div>
</div>
